...
export const assignNextAvailableDoctor = async (specialty) => {
    if (isFirebaseConfigured) {
        const q = query(
            collection(realDb, 'doctors'),
            where('specialization', '==', (specialty || '').toLowerCase()),
            orderBy('assignedPatientsCount', 'asc'),
            limit(1)
        );
        const snap = await getDocs(q);
        if (!snap.empty) {
        ...
        return { id: 'D001', name: 'Dr. Default (Assigned fallback)' };
    }
    const candidates = mockDB.doctors.filter(d =>
        (d.specialization || d.specialty || '').toLowerCase() === (specialty || '').toLowerCase()
    );
    const pool = candidates.length > 0 ? candidates : mockDB.doctors;
    let best = pool[0];
    let minLoad = Infinity;
    pool.forEach(d => {
        const load = mockDB.patients.filter(p => p.assignedDoctorId === d.id).length;
